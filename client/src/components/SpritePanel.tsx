    ...
    <Card className="h-full overflow-auto">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {spriteDefinitions.map((def) => (
                <button
                  ...
                  className={cn(
                    "p-2 border rounded hover-elevate active-elevate-2 transition-colors",
                    selectedSpriteDefId === def.id && "bg-accent border-primary ring-2 ring-primary/20"
                  )}
                  onClick={() => setSelectedSpriteDef(def.id)}
                >
                  <div className="text-xs truncate font-medium">{def.name}</div>
          ...
          {selectedDef ? (
            <div className="space-y-2 p-2 bg-accent/50 rounded-md border border-primary/20">
            ...
            <div className="text-xs text-muted-foreground text-center p-3 border border-dashed rounded-md">
              Click a sprite above to select it
            </div>
          )}
        </div>
